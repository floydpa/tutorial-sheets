...
class WsDividendsHL:
    ...
    def refresh(self):
        ...
        apply_formatting(self._forever_income, self.wsname())
...
class WsDividendsBySecurity:
    def __init__(self, forever_income):
        self._forever_income = forever_income
        self._workbook = forever_income.workbook()
        self._wsname   = WS_SEC_DIVIDENDS

        # Get dividends from 'hl' sheet
        hl = WsDividendsHL(forever_income)

        # Get dividends from 'fe' sheet
        fe = WsDividendsFE(forever_income)

        # Get other dividends (already aggregated)
        other = forever_income.worksheet_to_df(WS_OTHER_DIVIDENDS)
